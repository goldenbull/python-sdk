...
class TestTableAppender:
    ...
    def test_tableAppender_max_length_string_in_memory_table(self):
        ...
        data = '1' * (256 * 1024 - 1)
        tbname = 't_' + random_string(5)
        conn.run(f"{tbname}=table(100:0,[`str],[STRING])")
        appender = ddb.tableAppender(tableName=tbname, ddbSession=conn)
        df = pd.DataFrame({'id': [data]})
        appender.append(df)
        assert conn.run(f'strlen({tbname}[`str][0])') == 256 * 1024 - 1
        conn.close()
    ...
    def test_tableAppender_max_length_symbol_in_memory_table(self):
        ...
        data = '1' * (256 * 1024 - 1)
        tbname = 't_' + random_string(5)
        conn.run(f"{tbname}=table(100:0,[`str],[SYMBOL])")
        appender = ddb.tableAppender(tableName=tbname, ddbSession=conn)
        df = pd.DataFrame({'id': [data]})
        appender.append(df)
        assert conn.run(f'strlen({tbname}[`str][0])') == 256 * 1024 - 1
    ...
    def test_tableAppender_max_length_blob_in_memory_table(self):
        ...
        data = b'1' * 256 * 1024
        tbname = 't_' + random_string(5)
        conn.run(f"{tbname}=table(100:0,[`str],[BLOB])")
        appender = ddb.tableAppender(tableName=tbname, ddbSession=conn)
        df = pd.DataFrame({'id': [data]})
        appender.append(df)
        assert conn.run(f'strlen({tbname}[`str][0])') == 256 * 1024
        conn.close()
    ...
    def test_tableAppender_max_length_string_stream_table(self):
        ...
        data = '1' * (256 * 1024 - 1)
        tbname = 't_' + random_string(5)
        conn.run(f"{tbname}=streamTable(100:0,[`str],[STRING])")
        appender = ddb.tableAppender(tableName=tbname, ddbSession=conn)
        df = pd.DataFrame({'id': [data]})
        appender.append(df)
        assert conn.run(f'strlen({tbname}[`str][0])') == 256 * 1024 - 1
    ...
    def test_tableAppender_max_length_symbol_stream_table(self):
        ...
        data = '1' * (256 * 1024 - 1)
        tbname = 't_' + random_string(5)
        conn.run(f"{tbname}=streamTable(100:0,[`str],[SYMBOL])")
        appender = ddb.tableAppender(tableName=tbname, ddbSession=conn)
        df = pd.DataFrame({'id': [data]})
        appender.append(df)
        assert conn.run(f'strlen({tbname}[`str][0])') == 256 * 1024 - 1
    ...
    def test_tableAppender_max_length_blob_stream_table(self):
        ...
        data = b'1' * 256 * 1024
        tbname = 't_' + random_string(5)
        conn.run(f"{tbname}=streamTable(100:0,[`str],[BLOB])")
        appender = ddb.tableAppender(tableName=tbname, ddbSession=conn)
        df = pd.DataFrame({'id': [data]})
        appender.append(df)
        assert conn.run(f'strlen({tbname}[`str][0])') == 256 * 1024
